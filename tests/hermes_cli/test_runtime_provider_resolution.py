...
def test_bare_custom_does_not_trust_non_loopback_when_provider_not_custom(monkeypatch):
    ...
    with pytest.raises(AuthError, match="refusing to fall through to OpenRouter"):
...
def test_bare_custom_without_named_entry_still_falls_through(monkeypatch):
    ...
    local model.base_url trust-path behavior, unchanged by the fix."""
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
...
def test_bare_custom_without_endpoint_refuses_openrouter_placeholder(monkeypatch):
    from hermes_cli.auth import AuthError
